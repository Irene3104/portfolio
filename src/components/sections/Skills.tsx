        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-5 max-md:grid-cols-1 max-md:gap-3.5">
          {skillCategories.map((cat, i) => (
            <ScrollReveal
              key={cat.title}
              delay={0}
              className="rounded-[18px] border border-accent-1/8 bg-bg-card p-6 transition-all duration-300 hover:border-accent-1/20 hover:shadow-[0_4px_30px_rgba(34,211,238,0.06)]"
            >
              ...
              <div className="flex flex-wrap gap-2">
                {cat.items.map((item) => (
                  <span
                    key={item}
                    className="cursor-default rounded-full border border-accent-1/12 px-4 py-[7px] text-[1.05rem] text-text-secondary transition-all duration-300 hover:border-accent-1 hover:text-accent-1 hover:shadow-[0_0_12px_rgba(34,211,238,0.1)]"
                  >
                    {item}
                  </span>
                ))}
